...
(function() {
    ...
    function fixDeleteFunctionality() {
        ...
        window.deleteMemo = function(id) {
            ...
            if (Array.isArray(calendarMemos)) {
                // 구버전 배열 구조
                memo = calendarMemos.find(m => m.id == id);
            } else if (typeof calendarMemos === 'object') {
                // 신버전 날짜별 객체 구조
                for (const date in calendarMemos) {
                    if (Array.isArray(calendarMemos[date])) {
                        const foundMemo = calendarMemos[date].find(m => m.id == id);
                        if (foundMemo) {
                            memo = foundMemo;
                            break;
                        }
    ...
    function fixEditFunctionality() {
        ...
        window.editMemo = function() {
            ...
            // 메모 찾기 - 날짜별 그룹화된 데이터 처리
            const calendarMemos = JSON.parse(localStorage.getItem('calendarMemos') || '{}');
            let memo = null;
            let allMemos = [];

            // 날짜별로 그룹화된 메모 데이터 처리
            if (typeof calendarMemos === 'object' && !Array.isArray(calendarMemos)) {
                console.log('📅 날짜별 메모 데이터에서 검색');
                for (const date in calendarMemos) {
                    if (Array.isArray(calendarMemos[date])) {
                        allMemos = allMemos.concat(calendarMemos[date]);
                        const foundMemo = calendarMemos[date].find(m => m.id == window.currentMemoId);
                        if (foundMemo && !memo) {
                            memo = foundMemo;
                            console.log(`✅ ${date}에서 편집할 메모 발견:`, memo.title);
                        }
                    }
                }
            } else if (Array.isArray(calendarMemos)) {
                // 기존 배열 형태 - 호환성을 위해 유지
                allMemos = calendarMemos;
                memo = calendarMemos.find(m => m.id == window.currentMemoId);
            }
        ...
        function createEditModal(memo) {
            ...
            window.saveEditedMemo = function() {
                ...
                // 메모 업데이트 - 날짜별 객체 구조 지원
                const calendarMemos = JSON.parse(localStorage.getItem('calendarMemos') || '{}');
                let updated = false;

                if (Array.isArray(calendarMemos)) {
                    // 구버전 배열 구조
                    const memoIndex = calendarMemos.findIndex(m => m.id == window.currentMemoId);
                    if (memoIndex !== -1) {
                        calendarMemos[memoIndex].title = newTitle;
                        calendarMemos[memoIndex].content = newContent;
                        if (newTime && calendarMemos[memoIndex].isSchedule) {
                            calendarMemos[memoIndex].time = newTime;
                        }
                        updated = true;
                    }
                } else if (typeof calendarMemos === 'object') {
                    ...
                    for (const date in calendarMemos) {
                        if (Array.isArray(calendarMemos[date])) {
                            const memoIndex = calendarMemos[date].findIndex(m => m.id == window.currentMemoId);
                            if (memoIndex !== -1) {
                                calendarMemos[date][memoIndex].title = newTitle;
                                calendarMemos[date][memoIndex].content = newContent;
                                if (newTime && calendarMemos[date][memoIndex].isSchedule) {
                                    calendarMemos[date][memoIndex].time = newTime;
                                }
                                updated = true;
                                break;
                            }
