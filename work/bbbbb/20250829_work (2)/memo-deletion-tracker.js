...
(function() {
    ...
    // localStorage.setItem을 가로채서 복원 방지
    const originalSetItem = localStorage.setItem;
    localStorage.setItem = function(key, value) {
        if (key === 'calendarMemos') {
            try {
                const newMemos = JSON.parse(value);
                const originalCount = newMemos.length;
                const restoredMemos = [];
                
                // 삭제된 메모들을 필터링
                const filteredMemos = newMemos.filter(memo => {
                ...
                if (restoredMemos.length > 0) {
                    ...
                    restoredMemos.forEach((memo, i) => {
                        console.warn(`   ${i+1}. "${memo.title}" (ID: ${memo.id})`);
                    });
