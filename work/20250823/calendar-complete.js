...
(function() {
    ...
    function openMemoModal(date) {
        ...
        const memos = JSON.parse(localStorage.getItem(`memos_${date}`) || '[]');
    ...
    CalendarApp.saveCalendarMemo = function() {
        ...
        localStorage.setItem(`memos_${date}`, JSON.stringify(memos));
        input.value = '';
    ...
    CalendarApp.exportData = function() {
        ...
        for (let i = 0; i < localStorage.length; i++) {
            ...
            if (key.startsWith('memos_')) {
                data.memos[key] = JSON.parse(localStorage.getItem(key));
            }
        }
    ...
    CalendarApp.backupToGoogleDrive = function() {
        ...
        for (let i = 0; i < localStorage.length; i++) {
            const key = localStorage.key(i);
            if (key.startsWith('memos_')) {
                backupData.memos[key] = JSON.parse(localStorage.getItem(key));
            }
        }
    ...
    CalendarApp.restoreFromGoogleDrive = function() {
        ...
        for (const key in data.memos) {
            localStorage.setItem(key, JSON.stringify(data.memos[key]));
        }
