...
(function() {
    ...
    // 메모 추가
    function addMemo(title, content, date = null, htmlContent = null, attachedFiles = []) {
        const memo = {
            id: Date.now(),
            title: title,
            content: content,
            ...
            htmlContent: htmlContent, // 서식 정보 저장
            attachedFiles: attachedFiles || [] // 첨부 파일 저장
        };
    ...
    function refreshDateMemoList() {
        ...
        element.innerHTML = dateMemos.map(memo => {
            const hasFiles = memo.attachedFiles && memo.attachedFiles.length > 0;
            const fileIndicator = hasFiles ? `<span class="file-indicator" title="${memo.attachedFiles.length}개 파일 첨부">📎 ${memo.attachedFiles.length}</span>` : '';
    ...
    function replaceGlobalFunctions() {
        ...
        window.saveStickyMemo = function() {
            const editor = document.getElementById('stickyMemoInput');
            const inputText = editor?.textContent?.trim() || editor?.innerText?.trim();
            ...
            // 첨부 파일 가져오기
            const attachedFiles = window.currentAttachedFiles ? [...window.currentAttachedFiles.stickyMemo] : [];
            
            const memo = addMemo(title, content, todayStr, htmlContent, attachedFiles);
            ...
            // 첨부 파일 초기화
            if (window.currentAttachedFiles) {
                window.currentAttachedFiles.stickyMemo = [];
                const attachedFilesContainer = document.getElementById('stickyMemoAttachedFiles');
                if (attachedFilesContainer) {
                    attachedFilesContainer.innerHTML = '';
                }
            }
            
            console.log('💾 스티커 메모 저장 (서식 포함, 오늘 날짜):', memo.title, '날짜:', todayStr);
        };
        ...
        window.saveDateMemo = function() {
            ...
            // 첨부 파일 가져오기
            const attachedFiles = window.currentAttachedFiles ? [...window.currentAttachedFiles.dateMemo] : [];
            
            const memo = addMemo(title, content, MemoSystem.selectedDate, null, attachedFiles);
            ...
            // 첨부 파일 초기화
            if (window.currentAttachedFiles) {
                window.currentAttachedFiles.dateMemo = [];
                const attachedFilesContainer = document.getElementById('dateMemoAttachedFiles');
                if (attachedFilesContainer) {
                    attachedFilesContainer.innerHTML = '';
                }
            }
    ...
    function reattachEventListeners() {
        // 스티커 메모 저장 버튼
        const saveStickyBtn = document.getElementById('saveStickyMemo');
        if (saveStickyBtn) {
            // 기존 이벤트 리스너 제거
            saveStickyBtn.replaceWith(saveStickyBtn.cloneNode(true));
            const newSaveStickyBtn = document.getElementById('saveStickyMemo');
            if (newSaveStickyBtn) {
                newSaveStickyBtn.addEventListener('click', window.saveStickyMemo);
                console.log('🔄 스티커 메모 저장 버튼 이벤트 재등록 완료 (오늘 날짜용)');
            }
        }
        
        // 날짜 메모 저장 버튼
        const saveDateBtn = document.getElementById('saveDateMemo');
        if (saveDateBtn) {
            // 기존 이벤트 리스너 제거
            saveDateBtn.replaceWith(saveDateBtn.cloneNode(true));
            const newSaveDateBtn = document.getElementById('saveDateMemo');
            if (newSaveDateBtn) {
                newSaveDateBtn.addEventListener('click', window.saveDateMemo);
                console.log('🔄 날짜 메모 저장 버튼 이벤트 재등록 완료');
            }
        }
