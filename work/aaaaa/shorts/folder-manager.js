            ...
            folderDiv.addEventListener('drop', async (e) => {
                ...
                if (source === 'download') {
                    ...
                    if (this.selectedFiles.size > 1 && this.selectedFiles.has(fileName)) {
                        ...
                        for (const selectedFileName of filesToMove) {
                            try {
                                ...
                                this.downloadFiles = this.downloadFiles.filter(f => f.name !== selectedFileName);
                                this.selectedFiles.delete(selectedFileName);
                                successCount++;
                                
                                // 실시간 개수 업데이트
                                this.updateDownloadFileCount();
                                this.updateCategoryFileCount(category.name, 1); // 대상 카테고리에 파일 추가
                            } catch (error) {
                        ...
                        console.log(`선택 목록에서 제거: ${fileName}, 남은 선택: ${this.selectedFiles.size}개`);
                    ...
                    if (sourceCategory !== category.name) {
                        ...
                        // 카테고리 파일 개수 실시간 업데이트
                        this.updateCategoryFileCount(sourceCategory, -1); // 원본 카테고리에서 파일 제거
                        this.updateCategoryFileCount(category.name, 1); // 대상 카테고리에 파일 추가
                    } else {
    ...
    async moveToDownload(fileName, sourceCategory) {
        try {
            ...
            if (result.success) {
                ...
                // 실시간 업데이트는 드래그앤드롭/일괄이동 핸들러에서 처리됨
    ...
    async applyDownloadBatchMove() {
        ...
        for (const fileName of filesToMove) {
            try {
                ...
                // 실시간 개수 업데이트
                this.updateDownloadFileCount();
                this.updateCategoryFileCount(targetCategory, 1); // 대상 카테고리에 파일 추가
            } catch (error) {
        ...
        for (const fileName of this.selectedCategoryFiles) {
            try {
                ...
                if (response.ok) {
                    ...
                    // 실시간 파일 개수 업데이트
                    if (moveDestination === 'download') {
                        // 다운로드 폴더로 이동 시 로컬 데이터에 파일 추가
                        this.downloadFiles.push({ name: fileName });
                        this.updateDownloadFileCount();
                        this.updateCategoryFileCount(this.currentCategory, -1); // 원본 카테고리에서 파일 제거
                        console.log(`📥 로컬 데이터에 파일 추가: ${fileName}, 총 ${this.downloadFiles.length}개`);
                    } else {
                        // 카테고리 간 이동 시 파일 개수 업데이트
                        this.updateCategoryFileCount(this.currentCategory, -1); // 원본 카테고리에서 파일 제거
                        this.updateCategoryFileCount(targetCategory, 1); // 대상 카테고리에 파일 추가
                    }
                } else {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log('앱 초기화 완료. 테스트하려면 브라우저 콘솔에서 testRenameModal() 실행');
});
